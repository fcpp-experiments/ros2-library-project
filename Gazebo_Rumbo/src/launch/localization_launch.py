...
def print_values(context, params):
    map_var = LaunchConfiguration('map')
    params_var = LaunchConfiguration('params_file')
    value = map_var.perform(context)
    params_value = params_var.perform(context)
    print("Localization launch {0}".format(str(value)))
    print("Localization launch {0}".format(str(params_value)))
...
def generate_launch_description():
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(gazebo_bringup_dir, 'maps', 'library-new.yaml'),
